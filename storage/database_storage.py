...
class DatabaseStorage(StorageBase):
    ...
    def append(self, df_record: DataFrame, commit_every=1000, have_scd=False):
        logging.info(f"Commit sorage : {commit_every}")
        self.backend.append(self.table_name, df_record, commit_every=commit_every, have_scd=have_scd)
        return
    ...
    def get_record_range_v2(self, included_min_timestamp, included_max_timestamp, symbol_column=config.SYSTEM_SYMBOL_COL, timestamp_column=config.SYSTEM_TIMESTAMP_COL, target_symbols=None, filter_query=None, ):
        return self.backend.get_record_range_v2(self.table_name, included_min_timestamp, included_max_timestamp, symbol_column, timestamp_column, target_symbols, filter_query)
    ...
    def get_record_filter_query(
        ...
        return self.backend.get_record_filter_query(
